...
Cross-platform version (macOS / Linux / WSL).
...
Usage:
    python3 marlin_bridge.py init-session --task "my-task"
    python3 marlin_bridge.py write-heal --desc "..." --cmd "..." --error-file /tmp/err.txt --cwd /path
    python3 marlin_bridge.py read-response
    python3 marlin_bridge.py clear-heal
    python3 marlin_bridge.py update-step --step "3.5" --status "running"
    python3 marlin_bridge.py set-repo --path /path/to/repo
...
def gather_repo_context(repo_path: str) -> dict:
    ...
    if (repo / "setup.py").exists() or (repo / "pyproject.toml").exists():
        ...
        ctx["language"] = "ruby"
    elif (repo / "composer.json").exists():
        ctx["language"] = "php"
    elif (repo / "build.sbt").exists():
        ctx["language"] = "scala"
    elif (repo / "Package.swift").exists():
        ctx["language"] = "swift"
    elif (repo / "pubspec.yaml").exists():
        ctx["language"] = "dart"
    elif (repo / "mix.exs").exists():
        ctx["language"] = "elixir"
    elif (repo / "build.zig").exists():
        ctx["language"] = "zig"

    for cname in ["setup.py", "pyproject.toml", "setup.cfg", "package.json",
                   "Cargo.toml", "go.mod", "tox.ini", "Makefile",
                   "CONTRIBUTING.md", "Gemfile", "composer.json",
                   "build.sbt", "Package.swift", "pubspec.yaml", "mix.exs"]:
        cpath = repo / cname
        if cpath.exists():
